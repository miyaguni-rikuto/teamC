...
protected:
	// 各シーンが所有する情報
	std::vector<GameObject*>	create_list;	// 生成を行うオブジェクトリスト
	std::vector<GameObject*>	object_list;	// 更新と描画を行うオブジェクトリスト
	std::vector<GameObject*>	destroy_list;	// 破棄を行うオブジェクトリスト
...
public:
	...
	virtual eSceneType Update(const float& delta_second)
	{
		...
		for (int i = 0; i < object_list.size(); i++)
		{
			...
			for (int j = 0; j < object_list.size(); j++)
			{
				...
				HitCheckObject(object_list[i], object_list[j]);
			}
		}
	...
	virtual void HitCheckObject(GameObject* target, GameObject* partner)
	{
	}
